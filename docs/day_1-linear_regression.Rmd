...
theme_set(theme_light()) # Setting the default ggplot2 theme
...
Instead, we can also use the model parameters with geom_abline()

```{r}
ggplot() +
  aes(intercept = coef(acid_lm)[1], 
      slope = coef(acid_lm)[2]) +
  geom_abline()
```
...
- augment() adds important new columns to your data frames, such as the residuals (.resid), and predicted values corresponding your independent variables. These can be useful for residual diagnostics, and plotting model predictions.
...
One of the easiest way to calculate hereoscedasticity consistent standard errors is to use the `vcov.type` argument in the `tab_model()` function.
...
You can also use bootstrapping to estimate the residuals. 
There are several ways to do this. For e.g. the tab_model() function has a bootstrap argument.
...
You can also use "robust regression". Instead of OLS, robust regression uses iterated re-weighted least squares (IRLS) estimation. It is especially useful for smaller datasets with outliers and influentual cases.
...
Non-independence can take multiple forms. When your data is nested (e.g. you measure the math performance of students in a class in a school), you have a nested dependence. When you have multiple measurements from the same person, you have a crossed dependence. In general, these cases should be handled by using Linear Mixed-Effect Models (LMEMs). 
  ...
  For example, if you have a pre-post within subject design, you can use the change scores (i.e. post - pre values), and use it as an outcome variable in lm. https://learncuriously.wordpress.com/2021/06/18/pre-post-analysis/ 
  
For analyzing pre-post designs, you can either use the "Difference-in-Differences approach, or use the baseline value as a covariate. 
  
## Difference in differences approach
```{r}
# Dataset about effects of reality TV watching on self-esteem (source: https://teachpsychscience.org/),

reality_raw <- read_csv("data/reality.csv")

reality <-
  reality_raw %>% 
  mutate(se_change = se_posttest - se_pretest)

# In this case, we are not including any predictors, and investigate if the change score is different from zero.
